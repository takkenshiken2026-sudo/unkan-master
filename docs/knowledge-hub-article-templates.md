...
- 試験ガイド（学習法・制度読み物）: [guide-article-template.md](./guide-article-template.md)
- **立ち位置の正本:** [content-positioning.md](./content-positioning.md)
...
**詳細は [content-positioning.md](./content-positioning.md) を正本とする。** 概要のみ:
...
試験ガイドの genre **用語ハブ活用法** は「使い方」だけを書き、用語定義は `/terms/` に置く。
...
一覧ページは用語一覧と同型（検索・分野チップ・3列表）。**列見出しは種別ごとに次のとおり統一**します。

| タブ | 1列目 | 2列目 | 3列目 |
|------|-------|-------|-------|
| 用語解説 | 用語 | 分野 | 定義 |
| 比較・整理表 | 項目 | 分野 | 概要（`summary`） |
| 数値・期限早見表 | 項目 | 分野 | 概要（`summary`） |
| よくある誤答 | 項目 | 分野 | 概要（`summary`） |

比較・早見・誤答の3列目は、記事のサマリ（`summary` 列）を文章で表示します。
...
| `tags` | `;` 区切り |
| `summary` | 一覧3列目の**概要**・meta 用（記事サマリを文章で記載） |
