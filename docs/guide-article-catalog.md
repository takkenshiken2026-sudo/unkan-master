# 試験ガイド記事カタログ（50本以内の候補）

`data/guide_articles.csv` に追加するときの **slug / genre** の候補プールです。**本番は50本以内** — 資格に不要な行は入れず、下から必要分だけ選んでください。資格名・制度・数値は本文で差し替えます。
...
2. **分野別対策** は `site-config.json` の `fields` に合わせて `field-{id}-*` を必要最小限だけ置く（3分野なら各分野2〜3本が目安。合計50本以内に収める）。
...
上記は **候補プール**（約120 slug＋アフィリエイト10本）。**本番 CSV は50本以内** に収め、資格に不要なジャンル・分野別の細分化は省略してください。

- 分野が多い資格 → 各 `field-*` は **基礎＋過去問の2本** など最小セットに絞る
- 同ジャンル内の「対象者別」（初学者 / リスキリング / 短期合格）は **1本に統合** するか、最も検索される1本だけ残す
- アフィリエイトは **10本目安** だが、通常ガイドと合わせて **50本上限** を優先する
