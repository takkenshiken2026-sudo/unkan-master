...
# 1. brief 作成（shell テンプレをコピーし、商品・ASP URL を実値に）
...
# → products.*_url / price_yen / name。URL 未確定なら次へ進まない
